...
def breakdown_strategy(
    ...
    *,
    ...
    if len(c) < max(lookback + 1, rsi_len + 5):
        return (False, 0.0, "sell", [], None)
    ll = np.min(l[-(lookback + 1) : -1])
    broke = c[-1] < ll and c[-1] < o[-1]
    _rsi = rsi(c, rsi_len)
    rs = _rsi[-1]
    vol_ok = vol[-1] > np.mean(vol[-lookback:]) * 1.3
    score = 0.0
    ...
    if vol_ok:
        ...
        tags.append("vol_spike")
    if rs < 45:
        score += _norm(55 - rs, 55 - 45, 55 - 35) * 1.0
        tags.append("rsi_confirm")
    return (broke and (vol_ok or rs < 45), score, "sell", tags, float(ll))
...
def retrace_ema_strategy(
    ...
    *,
    ...
    ef = ema(c, fast)
    es = ema(c, slow)
    trend_down = ef[-1] < es[-1] and ef[-1] < ef[-5]
    touched = (h[-1] >= ef[-1] >= l[-1]) or abs(c[-1] - ef[-1]) / max(ef[-1], 1e-9) < 0.0025
    score = 0.0
    tags: List[str] = []
    if trend_down:
        score += 0.9
    ...
    if touched:
        score += 0.8
        tags.append("ema21_touch")
    return (trend_down and touched, score, "sell", tags, float(ef[-1]))
...
def momentum_dump_strategy(
    ...
    score = (body_ratio - 0.6) * 1.5 + (atr_ratio - 1.0) * 0.8
    score = max(0.0, score)
    tags = ["momentum_down"] if passed else []
    return (passed, score, "sell", tags, None)
...
def supertrend_flip_down_strategy(
    h: np.ndarray, l: np.ndarray, c: np.ndarray
) -> StrategyResult:
    if len(c) < 30:
        return (False, 0.0, "sell", [], None)
    st, dirn = supertrend(h, l, c, period=10, mult=3.0)
    flip_short = dirn[-1] == -1 and dirn[-2] == 1 and c[-1] < st[-1]
    score = 1.0 if flip_short else 0.0
    return (bool(flip_short), score, "sell", ["st_flip_down"], float(st[-1]))
...
def evaluate_all(
    ...
    # Short-biased (symmetrical)
    results["breakdown"] = breakdown_strategy(o, h, l, c, v)
    results["retrace"] = retrace_ema_strategy(o, h, l, c)
    results["momentum_down"] = momentum_dump_strategy(o, h, l, c)
    results["st_flip_down"] = supertrend_flip_down_strategy(h, l, c)
    return results
